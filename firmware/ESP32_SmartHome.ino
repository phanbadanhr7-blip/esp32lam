...
// ── NÚT BOOT: GIỮ 5 GIÂY ĐỂ XÓA WIFI ─────────────────────────────────────────────────
#define BTN_BOOT 9
...
void streamCallback(FirebaseStream data) {
  ...
  if (type == "boolean") {
    ...
    if (path == "/reset_wifi") {
      if (val == true) {
        Serial.println("[REMOTE-BOOT] Yeu cau xoa WiFi & Khoi dong lai tu xa!");
        // Dat lai flag ve false truoc khi restart de tranh vong lap boot
        Firebase.RTDB.setBool(&fbSet, "/devices/reset_wifi", false);
        delay(1000);
        wm.resetSettings();
        delay(1000);
        ESP.restart();
...
void initFirebase() {
  ...
  if (Firebase.RTDB.beginStream(&fbStream, "/devices")) {
    ...
    // Ghi các thông tin WiFi & IP lên Firebase
    Firebase.RTDB.setString(&fbSet, "/status/ssid", WiFi.SSID());
    Firebase.RTDB.setString(&fbSet, "/status/ip", WiFi.localIP().toString());
  } else {
    fbReady = false;
    streamActive = false;
...
// ═════════════════════════════════════════════════════════════════════════════════════
void loop() {
  bool currentBootPressed = (digitalRead(BTN_BOOT) == LOW);
  static bool lastBootPressed = false;
  if (currentBootPressed != lastBootPressed) {
    if (fbReady && Firebase.ready() && WiFi.status() == WL_CONNECTED) {
      if (Firebase.RTDB.setBool(&fbSet, "/status/boot_pressed", currentBootPressed)) {
        lastBootPressed = currentBootPressed;
        Serial.printf("[DASHBOARD] Cap nhat nut BOOT vat ly: %s\n", currentBootPressed ? "ON" : "OFF");
      }
    } else {
      lastBootPressed = currentBootPressed;
    }
  ...
  if (fbReady && Firebase.ready() && WiFi.status() == WL_CONNECTED) {
    if (millis() - lastHeartbeat > HEARTBEAT_MS) {
      ...
      // Đồng bộ hóa liên tục SSID, IP address và trạng thái nút BOOT của board thực tế
      Firebase.RTDB.setString(&fbSet, "/status/ssid", WiFi.SSID());
      Firebase.RTDB.setString(&fbSet, "/status/ip", WiFi.localIP().toString());
      Firebase.RTDB.setBool(&fbSet, "/status/boot_pressed", currentBootPressed);
    }
  }
